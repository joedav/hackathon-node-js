...
app.post("/clientes", (request, response) => {
  DbService.inserirCliente(request.body)
    ...
    .catch(erro => {
      ...
      response
        .status(500)
        .send("Ocorreu um erro ao inserir o cliente no banco de dados");
    });
});
  ...
  DbService.consultarClientePorId(request.params.id)
    .then(cliente => {
      if (cliente) {
        ...
        response
          .status(404)
          .send(`Cliente com ID ${request.params.id} não encontrado`);
      }
    })
    .catch(erro => {
      ...
      response
        .status(500)
        .send("Ocorreu um erro ao consultar cliente por id do banco de dados");
    });
});
